...
class GroundingEnv:
    ...
    def reset(self):
        ...
        if 'largest' not in self.instruction \
            ...
            self.object_ids = random.sample([x for x in range(len(self.objects))
                                        if x not in correct_objects], 4)
        else:
            self.object_ids = self.get_candidate_objects_superlative_instr(
                         chosen_correct_object)
            self.object_ids = [self.object_dict[x] for x in self.object_ids]

        assert len(self.object_ids) == 4

        self.object_ids.insert(self.correct_location, correct_object_id)
        ...
        [spawn_object(self.game, object_id, pos_x, pos_y) for
            ...
            zip(self.object_ids, object_x_coordinates, object_y_coordinates)]
    ...
    def step(self, action_id):
        ...
        self.time += 1
        reward, object_id = self.get_reward()
        ...
        return state, (reward, object_id), is_final
