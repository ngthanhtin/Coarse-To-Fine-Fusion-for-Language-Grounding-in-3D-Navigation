...
class BCNet(nn.Module):
    ...
    """
    def __init__(self, v_dim, q_dim, h_dim, h_out, act='PReLU', dropout=[.2,.5], k=3):
...
if __name__=='__main__':
    net = BCNet(512,512,256,2).cuda()
    x = torch.Tensor(1,36,512).cuda()
    y = torch.Tensor(1,14,512).cuda()
    out = net.forward(x,y)
    print(out.shape)