...
import numpy as np
import pandas as pd
import svcca.cca_core2 as cca_core
...
def fourier_ccas(conv_acts1, conv_acts2, return_coefs=False,
                 compute_dirns=False, verbose=True):
    ...
    for i in range(height):
      for j in range(width):
          results_dict = cca_core.get_cca_similarity(
              fft_acts1[:, i, j, :].T, fft_acts2[:,
                                                  i, j, :].T, compute_dirns=compute_dirns,
              verbose=verbose)
          # print(results_dict)
          # apply inverse FFT to get coefficients and directions if specified
          if return_coefs:
            results_dict["cca_coef1"] = np.fft.ifft2(
                results_dict["cca_coef1"])
            results_dict["cca_coef2"] = np.fft.ifft2(
                results_dict["cca_coef2"])
          else:
...
def fourier_ccas2(fft_acts1, fft_acts2, return_coefs=True,
                 compute_dirns=False, verbose=True):
    """Computes cca similarity between two conv layers with DFT.
    ...
    height1, width1 = fft_acts1.shape[1], fft_acts1.shape[2]
    height2, width2 = fft_acts2.shape[1], fft_acts2.shape[2]
    if height1 != height2 or width1 != width2:
      height = min(height1, height2)
      width = min(width1, width2)
      new_size = [height, width]
