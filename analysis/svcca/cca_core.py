...
# epsilon=0.,
def get_cca_similarity(acts1, acts2, epsilon=1e-7, threshold=0.98,
                       compute_coefs=True,
                       compute_dirns=False,
                       verbose=True):
  ...
  additional_val = 0.00000
  sigmaxx /= (xmax + additional_val)
  sigmayy /= (ymax + additional_val)
  sigmaxy /= (np.sqrt(xmax * ymax) + additional_val)
  sigmayx /= (np.sqrt(xmax * ymax) + additional_val)
  ...
  if compute_coefs:
    ...
    dtype=return_dict["coef_x"][0].dtype
    
    return_dict["full_coef_x"] = np.zeros((numx, numx),dtype=dtype)
    np.place(return_dict["full_coef_x"], x_mask,
             return_dict["coef_x"])
    return_dict["full_invsqrt_xx"] = np.zeros((numx, numx), dtype=dtype)
    np.place(return_dict["full_invsqrt_xx"], x_mask,
             return_dict["invsqrt_xx"])
    
    return_dict["coef_y"] = v
    return_dict["invsqrt_yy"] = invsqrt_yy
    return_dict["full_coef_y"] = np.zeros((numy, numy), dtype=dtype)
    np.place(return_dict["full_coef_y"], y_mask,
             return_dict["coef_y"])
    return_dict["full_invsqrt_yy"] = np.zeros((numy, numy), dtype=dtype)
