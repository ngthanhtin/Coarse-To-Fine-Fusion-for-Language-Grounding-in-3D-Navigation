...
def GradCAM():
    ...
    model = A3C_LSTM_GA(args, ae_model=ae_model).to(device)
    if args.auto_encoder:
        model.load_state_dict(torch.load('./saved/cf_convolve/train_easy_convolve_cf_ae', map_location=torch.device(device)))
    ...
    # print(model_list)
    auto_encoder_module = model_list[1]
    image_fn_1 = nn.Sequential(auto_encoder_module.conv1, auto_encoder_module.prelu)
    image_fn_2 = nn.Sequential(auto_encoder_module.conv2, auto_encoder_module.prelu)
    image_fn_3 = nn.Sequential(auto_encoder_module.conv3, auto_encoder_module.prelu)
    text_fn = nn.Sequential(model_list[3], model_list[4])

    # cfca attention 
    channels = [128,64,64]
    layers = nn.ModuleList()
    attn_linear_fn = model_list[5]
    for k in range(5):
        for i in range(len(channels)):
            layers.append(attn_linear_fn.layers[k*len(channels) + i])

    outtro_cnn = nn.Sequential(model_list[6], model_list[0], model_list[7], model_list[0])
    
    linear_fn = nn.Sequential(model_list[9], model_list[0])
    
    time_emb_fn = model_list[8]
    hc_fn = model_list[10]
    critic_fn = model_list[11]
    actor_fn = model_list[12]
